...
def handle_user_input(fuction_type, company_name):
    ...
    if fuction_type == "app_related_role":
        ...
        list_dataset = [
            company_name + "_" + i + "_performance" for i in performance_related_role
        ]
    ...
    return (
        bigquery_object_query,
        list_dataset,
        row_level_security_id_query_condition,
        table_id_query_condition,
        company_name,
    )
...
class Superset:
    ...
    list_table = [
        ...
        },
        {
            ...
            "table_schema": {"id": {"type": Integer}, "name": {"type": String}},
            "path": "security/roles/",
            "request_method": "GET",
            ...
            if not inspect(self.engine).has_table(table_name=table["table_name"]):
                try:
                    Table(
                        ...
                        metadata,
                        *(
                            Column(column_name, column_type["type"])
                            for column_name, column_type in table[
                                "table_schema"
                            ].items()
                        ),
                    )
    ...
    def _get_csrf_token(self, headers):
        ...
        response = requests.request(
            method=request_method, url=self._prepare_final_url(path), headers=headers
        )
        if self._handle_response_status_code(response=response):
            return {
    ...
    def _handle_superset_access_token_expire_time(self) -> dict:
        if (
            not self._access_token_and_expire_time
            or self._access_token_and_expire_time["access_token_expire_time"]
            < datetime.now()
        ):
            self._access_token_and_expire_time = (
                self._get_superset_access_token_using_login()
            )
    ...
    def _fetch_superset_api_all_page(self, table) -> Generator:
        ...
        while not pagination_complete:
            params_as_rison = (
                f"(page:{next_page['page']},page_size:{next_page['page_size']})"
            )
            ...
            if response.json()["count"] <= next_page["page"] * next_page["page_size"]:
                ...
                next_page = {"page": next_page["page"] + 1, "page_size": self.page_size}
            yield response
        ...
        for row in response.json()["result"]:
            ...
            for column_name, column_type in table["response_schema"].items():
                if column_type["type"] == object:
                    result.update(
                        **{
                            (column_name + "_" + k): row[column_name][k]
                    ...
                    for k, v in column_type["properties"].items():
                        result.update(
                            **{
                                column_name
                                + "_"
                                + k: ",".join(str(j[k]) for j in row[column_name])
                            }
                        )
                else:
                    result.update(
    ...
    def _check_data_between_superset_and_postgres(self):
        ...
        for table in self.list_table:
            ...
            print(
                f'Running check data between superset and postgres for table {table["table_name"]} \\n',
                flush=True,
            )
            if table["id"] == 2 or table["id"] == 5:
                print(
                    f'Cloning data from superset to postgres {table["table_name"]} \\n',
                    flush=True,
                )
                records = self._get_record(
                    parse_function=self._parse_response, table=table
                )
    ...
    def _prepare_dataset_information(self, list_dataset, table_id_query_condition):
        ...
        for dataset in list_dataset:
            ...
            list_dataset_with_information.append(
    ...
    def create_role(self, list_object, list_dataset, table_id_query_condition):
        self._check_data_between_superset_and_postgres()
        list_dataset_with_information = self._prepare_dataset_information(
            list_dataset=list_dataset, table_id_query_condition=table_id_query_condition
        )
        for object in list_object:
            object.update({"role_ids": [], "table_ids": []})
            for dataset in list_dataset_with_information:
                role_name = (
                    f"auto_{dataset['dataset_name'].upper()}_{object['object_name']}"
                )
                ...
                if not has_role:
                    ...
                    if self._handle_response_status_code(response=response):
                        ...
                        self._update_role_permission(
                            role_id, role_name, permission_view_menu_ids
                        )

                        # add role id and role name to object to create row_level_security
                        object["role_ids"].append((role_id))
                        object["table_ids"].extend(
                            (int(i) for i in dataset["table_id"].split(","))
                        )
    ...
    def _create_row_level_security(self, object):
        ...
        if not has_role:
            ...
            if self._handle_response_status_code(response=response):
                print(
                    f'Create role level security {object["role_level_security_name"]} successfully \\n',
                    flush=True,
                )
                time.sleep(1.5)

    def update_table_all_row_level_security(
        self, tables_schema, list_dataset, table_id_query_condition
    ):
        self._check_data_between_superset_and_postgres()
        ...
        tables_id_all_dataset: str = ",".join(
            i["table_id"] for i in list_dataset_with_information
        )
        ...
        list_row_level_security_as_dict = self._query_postgres(
            query, return_type="dict"
        )
        for object in list_row_level_security_as_dict:
            self._update_table_row_level_security(
                object=object, tables_id=tables_id_as_list
            )
            time.sleep(1.5)
    ...
    def _update_table_row_level_security(self, object, tables_id):
        ...
        if "auto" not in object["name"]:
            ...
            if "performance" in old_name:
                ...
                new_name = (
                    f"auto_{old_name[0].upper()}_{old_name[2]}_{old_name[1].lower()}"
                )
        else:
    ...
    def delete_roles(self):
        ...
        for role in object:
            self._delete_object(
                object_id=role["id"], object_name=role["name"], path=path
            )
            time.sleep(1.5)
    ...
    def delete_permission_views(self):
        ...
        for object in objects:
            self._delete_object(
                object_id=object["id"], object_name=object["name"], path=path
            )
            time.sleep(1.5)
    ...
    def _update_user_role(self, user_id, user_name, role_id, user_password):
        ...
        response = requests.request(
            ...
            headers=headers,
            cookies=cookies,
            json={"roles": role_id, "password": user_password},
        )
        if self._handle_response_status_code(response=response):
    ...
    def update_users_app_permission(self, objects, company_name, bigquery_connection):
        ...
        for object in list_object:
            ...
            list_mkt_and_new_role_id.append(
                {"marketer_name": object["marketer_name"], "new_role_id": new_role_ids}
            )
        ...
        for mkt_name in list_mkt_distict:
            ...
            for j in list_mkt_and_new_role_id:
                if mkt_name == j["marketer_name"]:
                    result["new_role_id"].extend(
                        int(id) for id in j["new_role_id"].split(",")
                    )
            list_mkt_and_new_role_id_distinct.append(result)

        # print(list_mkt_and_new_role_id_distinct, flush=True)
        ...
        for mkt in list_active_mkt:
            ...
            old_role_id_remain = []
            for i in zip(
                mkt["old_role_name"].split(","), mkt["old_role_id"].split(",")
            ):
                if ("auto_" + company_name.lower() + "_all_users") not in i[
                    0
                ].lower() and ("auto_" + company_name.lower() + "_mkt") not in i[
                    0
                ].lower():
                    old_role_id_remain.append(int(i[1]))
            mkt_and_old_role_remain.update(
                {
                    "marketer_name": mkt["username"],
                    "old_role_id_remain": old_role_id_remain,
                    "user_id": mkt["user_id"],
                }
            )
            list_mkt_and_old_id_remain_distinct.append(mkt_and_old_role_remain)

        # print(list_mkt_and_old_id_remain_distinct, flush=True)
        ...
        for i in list_mkt_and_old_id_remain_distinct:
            for j in list_mkt_and_new_role_id_distinct:
                if i["marketer_name"] == j["marketer_name"]:
                    list_final_role_id = (
                        list(set(j["new_role_id"])) + i["old_role_id_remain"]
                    )
                    # get user password
                    ...
                    user_password = next(
                        i
                        for i in bigquery_connection.get_object_from_bigquery(
                            query=user_password_query, start_date=None
                        )
                    ).get("password")
                    # update users role
                    # print(
                    #     f"user_id: {i['user_id']}, role_id: {list_final_role_id}",
                    #     flush=True,
                    # )
                    self._update_user_role(
                        user_id=i["user_id"],
                        user_name=i["marketer_name"],
                        role_id=list_final_role_id,
...
class Bigquery:
    ...
    def get_object_from_bigquery(self, query, start_date=None) -> Generator:
        ...
        if start_date:
            where_condition = (
                f' and cast(start_date as DATE) >= cast("{start_date}" as DATE)'
            )
        else:
            where_condition = ""
...
if __name__ == "__main__":
    ...
    (
        ...
        table_id_query_condition,
        company_name,
    ) = handle_user_input(fuction_type=fuction_type, company_name=com)

    bg = Bigquery()
    list_object = bg.get_object_from_bigquery(
        query=bigquery_object_query, start_date=start_date
    )
